...
TEXT_ABOUT = "Бот Эксодус создает условия для создания референтной сети доверия участников, оказывающих безвозмездную " \
...
TEXT_CONVENTION = f"*Условные обозначения в боте:*\n\n" \
...
TEXT_MENU = f"*Главное меню бота Эксодус*\n\n" \
            f"{MAN}*Профиль*\n" \
...
TEXT_HOW_START = f"*Как начать работу в боте.*\n\n" \
...
TEXT_CASE = f"*Возможные кейсы для использования бота Exodus*\n\n" \
...
TEXT_SUM_DIGIT = {'ru': 'Сумма должна быть только в виде цифр.', 'en': 'The amount should only be in the form of numbers.'}
