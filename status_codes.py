# orange_status
NEW_ORANGE_STATUS = 'NEW_ORANGE_STATUS'
APPROVE_ORANGE_STATUS = 'APPROVE_ORANGE_STATUS'
DECLINE_ORANGE_STATUS = 'DECLINE_ORANGE_STATUS'
NEW_ORANGE_STATUS_F = 'NEW_ORANGE_STATUS_F'
APPROVE_ORANGE_STATUS_F = 'APPROVE_ORANGE_STATUS_F'

# red status
NEW_RED_STATUS = 'NEW_RED_STATUS'
APPROVE_RED_STATUS = 'APPROVE_RED_STATUS'
DECLINE_RED_STATUS = 'DECLINE_RED_STATUS'
NEW_RED_STATUS_F = 'NEW_RED_STATUS_F'
APPROVE_RED_STATUS_F = 'APPROVE_RED_STATUS_F'

# intention
NEW_INTENTION = 'NEW_INTENTION'
INTENTION_APPROVED = 'INTENTION_APPROVED'
INTENTION_DECLINE = 'INTENTION_DECLINE'

# obligations
NEW_OBLIGATION = 'NEW_OBLIGATION'
OBLIGATION_APPROVED = 'OBLIGATION_APPROVED'
OBLIGATION_DECLINE = 'OBLIGATION_DECLINE'

# other
MONEY_HAS_BEEN_SENT = 'MONEY_HAS_BEEN_SENT'
REMIND_LATER = 'REMIND_LATER'
BEFORE_3_DAYS = 'BEFORE_3_DAYS'
LAST_REMIND = 'LAST_REMIND'

# closed
CLOSED = 'CLOSED'

# Коды первой версии

# Статусы, пока что не доконца продуманные
# 0 - отменённое намерение
# 1 - созданное намерение
# 2 - замороженое намерение
# 10 - отменённое обязательство
# 11 - созданное обязательство (намерение переведено в обязательство)
# 12 - деньги на обязательство отправлены, но не подтверждены получателем
# 13 - деньги подтверждены отправка и получение
# 15 - запрос получателя на исполнение (дай денег)

# events types

#
# frozen замороженное намерение (при красном статусе)
