    ...
    if not_executed_from != [] or not_executed_to != []:
        ...
        if not_executed_from != []:
            if lang == "ru":
                ...
                text += f"Please check and confirm {HANDSHAKE}:\n"
        ...
        if not_executed_to != []:
            if lang == "ru":
                ...
                text += f"\nRepeat notice of the performance {HANDSHAKE}:\n"
        ...
        if lang == "ru":
            ...
            text += f'\n\n{INPUT_NUMBER_USER["ru"]}'
        else:
            btn2 = types.KeyboardButton(text='Back')
            text += f'\n\n{INPUT_NUMBER_USER["en"]}'
        ...
        if lang == "ru":
            ...
            text = f"No unconfirmed reports {HANDSHAKE}"
    ...
    if lang == 'ru':
        if history_intention_from.count() != 0:
            ...
            for intention in history_intention_from:
                ...
                text_from += f'{intention.create_date.date().strftime("%d-%m-%Y")}     {int(intention.payment)} {intention.currency} вы {RIGHT_ARROW} {user_from.first_name} {user_from.last_name}\n'
        else:
        ...
        if history_intention_to.count() != 0:
            ...
            for intention in history_intention_to:
                ...
                text_to += f'{intention.create_date.date().strftime("%d-%m-%Y")}     {int(intention.payment)} {intention.currency} {user_to.first_name} {user_to.last_name} {RIGHT_ARROW} вам\n'
        else:
...
{text_from}'
        ...
        if history_intention_from.count() != 0:
            ...
            for intention in history_intention_from:
                user_from = read_exodus_user(intention.to_id)
                text_from += f'{intention.create_date.date().strftime("%d-%m-%Y")}     {int(intention.payment)} {intention.currency} you {RIGHT_ARROW} {user_from.first_name} {user_from.last_name}\n'
        else:
        ...
        if history_intention_to.count() != 0:
            ...
            for intention in history_intention_to:
                user_to = read_exodus_user(intention.from_id)
                text_to += f'{intention.create_date.date().strftime("%d-%m-%Y")}     {int(intention.payment)} {intention.currency} {user_to.first_name} {user_to.last_name} {RIGHT_ARROW} to you\n'
        else:
...
def show_other_socium(message, user_id):
    ...
    lang = read_user_language(message.chat.id)
    if lang == 'ru':
        bot_text = 'В сети участника:{}'.format(string_name) + f'\n\n{INPUT_NUMBER_USER["ru"]}'
        btn1 = types.KeyboardButton(text='Назад')
    else:
        bot_text = "In the participants network:{}".format(string_name) + f'\n\n{INPUT_NUMBER_USER["en"]}'
        btn1 = types.KeyboardButton(text='Back')
...
def show_my_socium(message):
    ...
    lang = read_user_language(message.chat.id)
    if lang == "ru":
        bot_text = 'В моей сети:{}'.format(string_name) + f'\n\n{INPUT_NUMBER_USER["ru"]}'
        btn1 = types.KeyboardButton(text='Назад')
    else:
        bot_text = 'In my network: {}'.format(string_name) + f'\n\n{INPUT_NUMBER_USER["en"]}'
        btn1 = types.KeyboardButton(text='Back')
    markup.row(btn1)
    msg = bot.send_message(message.chat.id, bot_text, reply_markup=markup, parse_mode="html",
                           disable_web_page_preview=True)
...
def expand_my_socium(message):
    ...
    if len(list_expand_socium) == 0:
        ...
        if lang == 'ru':
            bot_text = 'Расширение сети:{}'.format(string_name) + f'\n\n{INPUT_NUMBER_USER["ru"]}'
            btn1 = types.KeyboardButton(text='Назад')

        else:
            bot_text = 'Network extension: {}'. format (string_name) + f'\n\n{INPUT_NUMBER_USER["en"]}'
            btn1 = types.KeyboardButton(text='Back')
...
def all_check_int_obl_minus(message):
    number = message.text
    lang = read_user_language(message.chat.id)
    if number == 'Назад' or 'Back' in number:
        transactions_menu(message)
        return
    if not number.isdigit():
        if lang == 'ru':
            ...
            bot.send_message(message.chat.id, 'The number must be in the form of a number:')
    ...
    if intention is None or intention.status not in [1, 11]:
        if lang == 'ru':
            ...
            bot.send_message(message.chat.id, f'The entered number does not match the existing {HEART_RED} or {HANDSHAKE}:')
...
def for_other_check(message):
    ...
    if text == f'{HEART_RED} (0)':
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id, f'You don`t have {HEART_RED}')
        ...
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id, f'You don`t have {HANDSHAKE}')
...
remained {left_days} days:\n'.format(n=intent.intention_id,
    ...
    msg = bot.send_message(message.chat.id, bot_text_out)
...
def for_other_wizard_intention_check(message):
    ...
    if intention is None:
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id, f'The entered number does not match the existing ones {HEART_RED}:')
...
На сумму {payment} {currency}'.format(HEART_RED=HEART_RED,
        ...
        bot_text = 'Your {HEART_RED} in favor of {first_name} {last_name} {status}\n\
For the sum {payment} {currency}'.format(HEART_RED=HEART_RED,
                                              intention_id=intention_id,
                                              first_name=user_to.first_name,
                                              last_name=user_to.last_name,
                                              status=status,
    ...
    if show_back:
        if lang == 'ru':
            ...
            btn6 = types.KeyboardButton(text='Back')
...
def edit_intention_check(message):
    ...
    if payment == 'Назад' or 'Back' in payment:
        intention_for_needy(message, reminder_call=False, intention_id=None)
        return
...
def cancel_intention(message, intention_id):
    ...
    if lang == 'ru':
        ...
        bot_text = f"You want to cancel your {HEART_RED} participant {user_to.first_name} {user_to.last_name} on {intention.payment} {intention.currency}?"
        btn1 = types.KeyboardButton(text='No')
        btn2 = types.KeyboardButton(text='Yes')
...
def cancel_intention_check(message, intention_id):
    ...
    if lang == 'ru':
        ...
        bot_text = f"Your {HEART_RED} participant {user_to.first_name} {user_to.last_name} on {intention.payment} {intention.currency} cancelled."
    ...
    if text == 'Нет' or 'No' in text:
        ...
        if lang_to == 'ru':
            ...
            text_for_needy = '{} {} canceled their {} in Your favor for the amount of {} {}'.format(
                user_from.first_name, user_from.last_name, HEART_RED, intention.payment, intention.currency)
        ...
        for row in list_needy_id:
            try:
                lang_to_all = read_user_language(row)
                if lang_to_all == 'ru':
                    ...
                    text_for_all = '{} {} canceled their {} {} {} for the amount of {} {}'.format(
                        user_from.first_name, user_from.last_name, HEART_RED, user_to.first_name, user_to.last_name,
                        intention.payment, intention.currency)
...
def for_other_wizard_obligation_check(message):
    ...
    lang = read_user_language(message.chat.id)
    if obligation_number == 'Назад' or obligation_number == 'Back':
        for_other_wizard(message)
        return
    ...
    if intention is None:
        if lang == 'ru':
            msg = bot.send_message(message.chat.id, f'{TEXT_NOT_FIND["ru"]} {HEART_RED}:')
        else:
            msg = bot.send_message(message.chat.id, f'{TEXT_NOT_FIND["en"]} {HEART_RED}:')
...
def obligation_for_needy_check(message, intention_id):
    ...
    if text == 'Да, я исполнил обязательство' or 'Yes' in text:
        ...
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id, 'Select menu item')
...
{user_to.first_name} {user_to.last_name} отправлено уведомление, что {intention.payment} {HANDSHAKE} исполнено."
        ...
        bot_text = f"Thanks!\n\
{user_to.first_name} {user_to.last_name} a notification was sent that {intention.payment} {HANDSHAKE} was executed."
    ...
    lang = read_user_language(user_to.telegram_id)
    if lang == 'ru':
        bot_text_from = f"{user_from_notif.first_name} {user_from_notif.last_name} исполнил в вашу пользу {intention.payment}{HANDSHAKE}.\n\
Пожалуйста, проверьте и подтвердите {HANDSHAKE}{RIGHT_ARROW}{LIKE}"
    else:
        bot_text_from = f"{user_from_notif.first_name} {user_from_notif.last_name} executed in your favor {intention.payment}{HANDSHAKE}.\n\
Please check and confirm {HANDSHAKE}{RIGHT_ARROW}{LIKE}"
...
def all_check_int_obl_plus(message):
    ...
    if not number.isdigit():
        if lang == 'ru':
            bot.send_message(message.chat.id, TEXT_NUMBER_DIGIT['ru'])
        else:
            bot.send_message(message.chat.id, TEXT_NUMBER_DIGIT['en'])
    ...
    if intention is None or intention.status not in [1, 11]:
        if lang == 'ru':
            bot.send_message(message.chat.id, f'{TEXT_NOT_FIND["ru"]} {HEART_RED} или {HANDSHAKE}:')
        else:
            bot.send_message(message.chat.id, f'{TEXT_NOT_FIND["en"]} {HEART_RED} or {HANDSHAKE}:')
...
def new_check_intention_send(message, intention):
    ...
    if text == 'Назад' or 'Back' in text:
        transactions_menu(message)
        return
    elif 'Попросить' in text or 'Ask' in text:
        to_id = intention.to_id
        from_id = intention.from_id
        lang_from = read_user_language(from_id)
        lang_to = read_user_language(to_id)
        user_to = read_exodus_user(to_id)
        if lang_to == 'ru':
            bot.send_message(to_id, f'Отправлен запрос на {HEART_RED} {RIGHT_ARROW} {HANDSHAKE}')
        else:
            bot.send_message(to_id, f'Sent a request to {HEART_RED} {RIGHT_ARROW} {HANDSHAKE}')
        ...
        if lang_from == 'ru':
            ...
            bot.send_message(from_id,
                             f'Request {HEART_RED} {RIGHT_ARROW} {HANDSHAKE} for {user_to.first_name} {user_to.last_name} for the sum {intention.payment}')
...
def for_my_check(message):
    ...
    if text == f'{HEART_RED} (0)':
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id, f'You dont have {HEART_RED}')
        ...
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id, f'You dont have {HANDSHAKE}')
...
def intention_for_me_check(message):
    ...
    if obligation_number == 'Назад' or obligation_number == 'Back':
        for_my_wizard_intention(message)
        return
    msg = bot.send_message(message.chat.id, 'Чтобы вернуться назад нажмите назад')
    bot.register_next_step_handler(msg, intention_for_me_check)
    return
...
def for_my_wizard_obligation_check(message):
    ...
    if intention is None:
        if lang == 'ru':
            msg = bot.send_message(message.chat.id, f'{TEXT_NOT_FIND["ru"]} {HANDSHAKE}:')
        else:
            msg = bot.send_message(message.chat.id, f'{TEXT_NOT_FIND["en"]} {HANDSHAKE}:')
...
def for_me_obligation_check(message, obligation_id):
    ...
    text = message.text
    # bot.delete_message(message.chat.id, message.message_id)
    if text == 'Запрос на исполнение' or 'Request' in text:
...
def obligation_to_execution(message, obligation_id):
    ...
    if lang == 'ru':
        ...
        bot_text = f'Participant {user.first_name} {user.last_name} a notification was sent with a request to execute ' \
                f'{HANDSHAKE} for the amount {intention.payment} {intention.currency}.'
...
def keep_obligation(message, obligation_id):
    ...
    if lang == 'ru':
        ...
        bot_text = f'{HANDSHAKE} participant {user.first_name} {user.last_name} for ' \
                   f'the sum  {intention.payment} {intention.currency} it will be stored with you until you make a decision.\n' \
                   f'You can view all {HANDSHAKE} in the "organizer" section of the main menu'
...
def not_executed_wizard(message):
    ...
    lang = read_user_language(message.chat.id)
    if lang == 'ru':
        bot_text = f'Неисполненными считаются те {HANDSHAKE}, которые не подтвердил получатель.'
        btn3 = types.KeyboardButton(text='Назад')
    else:
        bot_text = f'The {HANDSHAKE} that the recipient did not confirm is considered not executed.'
...
def not_executed_wizard_to_me_check(message):
    ...
    if intention_number == 'Назад' or 'Back' in intention_number:
        not_executed_wizard(message)
        return
    if not intention_number.isdigit():
        if lang == 'ru':
...
Реквизиты: {req_name} {req_value}"  # TODO реквезиты
        ...
        if requisites == []:
            ...
            req_name = requisites[0].name
            req_value = requisites[0].value
        bot_text = f"I didn't confirm the executed {HANDSHAKE} in my favor:\n\
        \n\
Date: {intention.create_date.strftime('%d %B %Y')}\n\
Sender: {user.first_name} {user.last_name} {get_status(user.status)}\n\
Sum: {intention.payment} {intention.currency}\n\
Requisites: {req_name} {req_value}"  # TODO реквезиты
        btn1 = types.KeyboardButton(text="I received this amount")
        btn2 = types.KeyboardButton(text="Repeated request for execution")
        btn3 = types.KeyboardButton(text='Back')
...
def executed_not_confirm_me_check(message):
    ...
    if text == 'Назад' or 'Back' in text:
        not_executed_wizard_to_me(message)
        return
    if 'Я получил эту сумму' in text or 'received' in text:
        executed_confirm(message)
        return
    if text == 'Повторный запрос на исполнение' or 'Repeated' in text:
...
Sum: {intention.payment} {intention.currency}\n\
Requisites: {req_name} {req_value}"  # TODO реквезиты
        btn1 = types.KeyboardButton(text="Yes, I received")
        btn2 = types.KeyboardButton(text='Back')
...
def executed_confirm_check(message):
    ...
    if text == 'Назад' or 'Back' in text:
        executed_not_confirm_me(message)
        return
    if text == 'Да, я получил' or 'Yes' in text:
...
Реквизиты: {req_name} {req_value}"  # TODO реквезиты
        ...
        if requisites == []:
            ...
            req_name = requisites[0].name
            req_value = requisites[0].value
        bot_text = f"My {HANDSHAKE} was not confirmed:\n\
        \n\
Date: {intention.create_date.strftime('%d %B %Y')}\n\
Time: {intention.create_date.strftime('%I:%M%p')}\n\
Recipient: {user.first_name} {user.last_name} {get_status(user.status)}\n\
Sum: {intention.payment} {intention.currency}\n\
Requisites: {req_name} {req_value}"  # TODO реквезиты
        btn1 = types.KeyboardButton(text="I sent this amount")
        btn2 = types.KeyboardButton(text='Back')
...
def executed_not_confirm_check(message):
    ...
    if text == 'Назад' or 'Back' in text:
        not_executed_wizard_for_all(message)
        return
    if text == 'Я отправил эту сумму' or 'sent' in text:
...
def executed_was_sent(message):
    ...
    if lang == 'ru':
        ...
        bot_text = f"Thanks! The recipient {user.first_name} {user.last_name} "\
                f"will be notified that the money has been sent."
    bot.send_message(message.chat.id, bot_text)
...
def members_menu_profile_link(message, member_id, name_return_func=""):
    ...
    if user.status == 'green':
        bot_text = f'{MAN} {user.first_name} {user.last_name} {GREEN_BALL}'
...
def welcome_base(message):
    ...
    if referral[0] != '':
        ...
        if lang == 'ru':
            ...
            bot_text = '{} {} invites you to help {} {}'.format(user_from.first_name,
                                                                user_from.last_name,
                                                                user_to.first_name,
                                                                user_to.last_name)
...
def start_orange_invitation(message, user_to, event_id=None, ref=None):
    ...
    if intention is not None:
        if lang == 'ru':
            ...
            bot_text = f'You are already helping the participant {user.first_name} {user.last_name}.'
...
def orange_invitation_wizard_check(message, event_id=None):  # ------------------ TODO
    ...
    if lang == "ru":
        ...
        bot_text = f'Your {HEART_RED} is accepted'
...
def show_all_members_check(message):
    bot.delete_message(message.chat.id, message.message_id)
    if message.text == 'Назад' or 'Back' in message.text:
...
({int(right_sum)}{HELP} from {days_end} days)'
    ...
    lang_for_u = read_user_language(user.telegram_id)
    if lang_for_u == 'ru':
...
def red_status_wizard(message):
    ...
    if lang == 'ru':
        ...
        bot_text = f'{RED_BALL} {user.max_payments}, remained {days_end} days'
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        btn1 = types.KeyboardButton(text=f'Change data {MONEY_BAG}')
        user_status = user.status
        if 'orange' in user_status:
            btn2 = types.KeyboardButton(text=f'Go back to {ORANGE_BALL}')  # orange
        if 'green' in user_status:
            btn2 = types.KeyboardButton(text=f'Go back to {GREEN_BALL}')  # green
        btn3 = types.KeyboardButton(text='Back')
    markup.row(btn1, btn2, btn3)
...
def red_status_wizard_check(message):
    bot.delete_message(message.chat.id, message.message_id)
    text = message.text
    if 'Изменить данные' in text or 'Change' in text:
        edit_red_data(message)
    elif 'Вернуться' in text or 'Go' in text:
...
Пожалуйста подтвердите действие', reply_markup=markup)
        ...
        btn2 = types.KeyboardButton(text='No')
        markup.row(btn1, btn2)
        msg = bot.send_message(message.chat.id, f'{RED_BALL} You are going to change the data {MONEY_BAG}\n\
Please confirm that it is valid', reply_markup=markup)
...
def check_edit_red_data(message):
    ...
    if 'Да' in text or 'Yes' in text:
        ...
        if lang == 'ru':
            ...
            msg = bot.send_message(message.chat.id,
                               f'{RED_BALL} Enter the amount of {MONEY_BAG} that you need to dial urgently',
                               reply_markup=markup)
...
def edit_red_data_day(message, new_sum):
    ...
    if not days.isdigit():
        if lang == 'ru':
            ...
            msg = bot.send_message(chat_id,
                               'The number of days must be in the form of numbers. Enter the number of days you need to collect this amount:')
        bot.register_next_step_handler(msg, edit_red_data_day, new_sum)
        return
...
Необходимая сумма: {new_sum} за {days} дней'
        ...
        bot_text = f'{RED_BALL}Please check the entered data:\n\
Required amount: {new_sum} for {days} days'

        btn1 = types.KeyboardButton(text=f'Change data {MONEY_BAG}')
        btn2 = types.KeyboardButton(text='Cancel')
        btn3 = types.KeyboardButton(text='Save status')
...
def edit_red_data_final(message, new_sum, days):
    ...
    elif text == 'Сохранить статус' or 'Save' in text:
        if lang == 'ru':
            bot.send_message(message.chat.id, 'Настройки сохранены')
        else:
            bot.send_message(message.chat.id, 'The settings are saved')
...
def check_red_edit_wizard(message):
    ...
    if lang == 'ru':
        ...
        bot_text = f'Вы собираетесь сменить статус на {RED_BALL}\n\
Пожалуйста подтвердите смену статуса'
    else:
        btn1 = types.KeyboardButton(text='Yes, change')
        btn2 = types.KeyboardButton(text='No, return back')
        bot_text = f'You are going to change your status to {RED_BALL}\n\
Please confirm the status change'
    markup.row(btn1, btn2)
    msg = bot.send_message(message.chat.id, bot_text, reply_markup=markup)
...
def check_answer_red_wizard(message):
    ...
    text = message.text
    user = read_exodus_user(message.chat.id)
    if text == 'Да, изменить' or 'Yes' in text:
        red_edit_wizard(message)
    elif text == 'Нет, вернуться назад' or 'No' in text:
        if user.status == 'orange':
            green_red_wizard(message)
...
def red_edit_wizard(message):
    ...
    if lang == 'ru':
        ...
        msg = bot.send_message(message.chat.id, 'Enter the amount in {} that you need:'.format(user.currency),
                           reply_markup=markup)
...
def red_edit_wizard_step3(message):
    ...
    if not days.isdigit():
        if lang == 'ru':
            msg = bot.send_message(chat_id,
                               'Кол-во дней должны быть в виде чисел. Введите кол-во дней, в течении которых вам необходимо собрать эту сумму:')
        else:
            msg = bot.send_message(chat_id,
                               'The number of days must be in the form of numbers. Enter the number of days you need to collect this amount:')
        bot.register_next_step_handler(msg, red_edit_wizard_step3)
...
Все пользователи, которые связаны с вами внутри Эксодус бота, получат уведомление.'
        ...
        bot_text = f'Please check the entered data:\n\
            \n\
Status: {RED_BALL}\n\
During: {user.days}\n\
Necessary amount: {user.max_payments} {user.currency}'
        bot.send_message(message.chat.id, bot_text)
        ...
        btn3 = types.KeyboardButton(text='Save status')
        markup.row(btn1, btn3, btn2)
        bot_text = 'Do you want to change your status and publish this data?\n\
            \n\
All users who are connected to you inside the Exodus bot will receive a notification.'
    msg = bot.send_message(message.chat.id, bot_text, reply_markup=markup)
        ...
        for row in list_needy_id:
            try:
                ...
                if lang == 'ru':
                    ...
                    bot.send_message(row, '{} {} changed the status to {}'.format(telegram_name.first_name,
                                                                         telegram_name.last_name, RED_BALL))
            except:
                continue
...
Все пользователи, которые связаны с вами внутри Эксодус бота, получат уведомление.', reply_markup=markup)
            ...
            bot.send_message(message.chat.id,
                             f'Your status is returned to {ORANGE_BALL}')
            # link = user.link
            if 'red' in user.status:
                payments = user.min_payments
            else:
